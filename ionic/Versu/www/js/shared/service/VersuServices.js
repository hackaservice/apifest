...
.factory('TopicService', function($http) {
    ...
    var topics = [
        ...
        function getMyTopics(userTwitterData, callback) {
            $http.get(URL_PAGE + '/topics/' + userTwitterData.name).then(function(response){
                callback(response.data);
            });
        }
    ...
    return {
        ...
        ,
        getMyTopics: function(userTwitterData, callback) {
            return getMyTopics(userTwitterData, callback);
        }
    };
})
